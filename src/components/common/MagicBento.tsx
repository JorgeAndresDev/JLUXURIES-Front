...
// GSAP will be imported dynamically at runtime (avoids hard dependency for builds)
...
export const ParticleCard: React.FC<{
  ...
  useEffect(() => {
    // try to dynamically import gsap at runtime; if not present, component will still render without the animations
    let mounted = true;
    (async () => {
      try {
        ...
        if (!mounted) return;
        gsapRef.current = (mod as any).gsap || (mod as any).default || mod;
      } catch (e) {
        // ignore — gsap not installed
      }
    })();
    return () => {
      mounted = false;
  ...
  const animateParticles = useCallback(() => {
    ...
    memoizedParticles.current.forEach((particle, index) => {
      const timeoutId = window.setTimeout(() => {
        ...
        gsapRef.current?.fromTo(clone, { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.3, ease: 'back.out(1.7)' });

        gsapRef.current?.to(clone, {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (enableMagnetism) {
        ...
        magnetismAnimationRef.current = gsapRef.current?.to(element, {
          x: magnetX,
          y: magnetY,
          duration: 0.3,
          ease: 'power2.out'
...
export const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (!mouseInside) {
        gsapRef.current?.to(spotlightRef.current, {
          opacity: 0,
          duration: 0.3,
          ease: 'power2.out'
        });
